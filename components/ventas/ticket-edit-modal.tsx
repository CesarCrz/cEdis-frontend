...
import { useUpdateVenta } from "@/hooks/use-ventas"
...
export function TicketEditModal({ open, onClose, cedisId, ticket }: TicketEditModalProps) {
  ...
  // Build initial items from existing ticket
  function buildInitialItems(): CreateTicketFormData["items"] {
    const existing = (ticket.items ?? ticket.partidas ?? []) as Array<{
      insumo_id: string
      cantidad: number
      unidad_id?: string
      unidad?: { id: string } | null
      precio_unitario: number
    }>
    if (existing.length === 0) {
      return [{ insumo_id: "", cantidad: 0, unidad_id: "", precio_unitario: 0 }]
    }
    return existing.map((it) => ({
      insumo_id: it.insumo_id,
      cantidad: Number(it.cantidad),
      unidad_id: it.unidad_id ?? (it.unidad as { id: string } | null)?.id ?? "",
  ...
  async function onSubmit(values: CreateTicketFormData) {
    const res = await updateVenta.mutateAsync({
      id: ticket.id,
      data: { items: values.items, notas: values.notas },
    })
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader className="shrink-0">
          <DialogTitle>Editar ticket {ticket.folio}</DialogTitle>
        </DialogHeader>

        <div className="flex-1 min-h-0 overflow-y-auto">
          <div className="pr-4 pl-1">
            <Form {...form}>
              <form id="ticket-edit-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pb-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground border rounded-md px-3 py-2 bg-muted/40">
                      {ticket.cliente?.nombre ?? ticket.cliente_id}
                    </p>
                  </div>
                  <FormField
        ...
        <DialogFooter className="gap-2 pt-4 border-t border-border shrink-0">
          <Button type="button" variant="outline" onClick={onClose} disabled={updateVenta.isPending}>Cancelar</Button>
          <Button form="ticket-edit-form" type="submit" disabled={updateVenta.isPending}>
            {updateVenta.isPending ? "Guardando..." : "Guardar cambios"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
